...
export const getCategory = (targetTech: Technology): Category[] => {
  ...
  const categories = companies
    ...
    .filter((it) =>
      it.technologies.find((tech) => tech.name === targetTech.name)
    )
...
export const getAllCategories = (market: MarketsType): Category[] => {
  ...
  const result = categoryIds
    ...
    .map((category) => {
      ...
      category.technologies = Array.from(techs).map(
        (techName) =>
          categories
            .flatMap((it) => it.technologies)
            .find((it) => it.name === techName)!!
      )
      return category
    })
